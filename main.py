...
# Importe der benÃ¶tigten Module
...
def main():
    ...
    try:
        ...
        if raw_data.empty:
            raise ValueError("Keine Rohdaten von yfinance erhalten. ÃœberprÃ¼fen Sie Ticker, Internet oder API-Limits.")
        
        print(f"âœ… Rohdaten erfolgreich heruntergeladen. {len(raw_data)} Kerzen.")
        ...
        print(f"âœ… Rohdaten erfolgreich vorbereitet. {len(original_data)} Kerzen verbleiben.")

        # 2. Zentrale Pipeline aufrufen (inklusive Debug-Ausgaben)
        print("â³ Starte zentrale Workflow-Pipeline (inkl. Kontroll-CSV-Ausgaben)...")
        ha_data, arms = workflow_pipeline(original_data)

        if ha_data.empty:
            raise ValueError("Heikin-Ashi Daten konnten nicht berechnet werden.")

        print(f"âœ… Heikin-Ashi Daten erfolgreich berechnet. {len(ha_data)} HA-Kerzen.")
        print(f"âœ… {len(arms)} Trendarme erkannt.")

        # Optional: Heikin-Ashi-Farbe berechnen (falls benÃ¶tigt)
        ...
        print(f"â„¹ï¸ Starte Validierung der Trendarme auf Heikin-Ashi Daten...")
        arm_container.validate_arms(ha_data) 
        validated_arms = [arm for arm in arm_container.arms if arm.validated]
        print(f"âœ… Validierung abgeschlossen. Ergebnisse in 'output/arm_validation_debug.txt'.")
        ...
        print("1. Internetverbindung/VPN prÃ¼fen.")
        print("2. Ticker auf BTC-USD oder ein anderes gÃ¤ngiges Symbol testen.")
        print("3. Das angefragte Zeitintervall kÃ¶nnte zu groÃŸ oder zu klein sein fÃ¼r yfinance.")
